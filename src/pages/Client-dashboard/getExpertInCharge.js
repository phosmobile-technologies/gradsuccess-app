...
class getExpertInCharge extends Component {
  ...
  render() {
    return (
      <Query query={GET_EXPERT_IN_CHARGE} variables={{ id: this.props.id }}>
        {({ loading, error, data }) => {
          if (loading)
            return (
              ...
              {data.getExpertInCharge.length !== 0 ? (
                <div className="expert_in_charge_board">
                  <div>
                    ...
                    <p>
                      <span>Name:</span>{" "}
                      {data.getExpertInCharge[0].first_name +
                        " " +
                        data.getExpertInCharge[0].last_name}
                    </p>
                    <p>
                      <span>Email:</span> {data.getExpertInCharge[0].email}
                    </p>
                  </div>
                </div>
              ) : (
