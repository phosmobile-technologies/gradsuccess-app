...
import { MESSAGE_MAIL } from "../../../api/sendMailEndpoint"
...
export default class leaveAMessageForm extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      messages: [
        {
          messageBody:
            "Welcome to Gradsucess... leave a for a message for an expert",
        },
      ],
      typedText: "",
      expertInChargeID: "",
  ...
  formSubmitted(data) {
    ...
    let receiver_id = ""
    let from = ""
    let url = MESSAGE_MAIL
    document.getElementById("submittedSuccess").style.display = "block"

    if (data.createMessages.client_id === sender_id) {
      receiver_id = data.createMessages.expert_id
      from = "Client"
    } else {
      receiver_id = data.createMessages.client_id
      from = "Expert"
    }

    let message_notification_id = {
    ...
    fetch(url, {
      ...
      method: "post",
      body: JSON.stringify(message_notification_id),
    })
  ...
  onChange(e) {
    ...
    var complete = ""
    let fileRef =
      "upload_files/" + CVName + "_" + timeSubmitted + "_" + file.name
      ...
      (complete = () => {
        storageRef
          ...
          .then(url => {
            fileLink = url
            uploaded = true
            document.getElementById("submittedInprogress").style.display =
              "none"
            document.getElementById("attachedDocument").src = AttachedFile

            setTimeout(() => {
              if (document.getElementById("submittedSuccess") != null) {
                document.getElementById("submittedSuccess").style.display =
                  "none"
              }
              this.setState({
                fileRef: fileRef,
      ...
      <div>
        ...
        {this.state.expertInChargeID === "empty" ? (
          <div className="chat_component">
            <p className="messagingNotAvailable">
              Messaging is not available at the moment, Your Application has not
              been assigned to an expert yet
            </p>
          </div>
        ) : (
          <div className="chat_component">
                                ...
                                >
                                  {messageInstance.message_type === "image" ? (
                                    <div className="display_message_upload_exist">
                                      <img
                                        className="up_exist"
                                        src={messageInstance.attachment_ref}
                                      />
                                      <div>
                                        <a
                                          target="_blank"
                                          href={messageInstance.attachment_ref}
                                        >
          ...
          >
            {(sendMessage, { data, loading, error }) => (
              <div className="loader-wrapper">
                ...
                <div
                  id="submittedInprogress"
                  className="sending_message_notification2"
                >
                  Uploading...
                </div>
                <form
                ...
                >
                  ...
                  <button
                    type="submit"
                    disabled={this.state.typedText === "" ? true : false}
                  >
                    <img src={sendIcon} alt="Logo" />
                  </button>
                </form>
                {loading && (
                  <div className="sending_message_notification">Sending...</div>
                )}
                {error && (
