...
import { ASSOCIATE_RATING } from "../../../graphql/queries"
            ...
            <section className="details-container">
              <div className="details-main">
                ...
                {this.state.data.email && (
                  <FieldSetView title="Email" value={this.state.data.email} />
                )}
                {this.state.data.phone && (
                  <FieldSetView title="Phone" value={this.state.data.phone} />
                )}
                {this.state.associate_bio && (
                  <div>
                    ...
                    {this.state.associate_bio.ielts && (
                      <FieldSetView
                        title="IELTS"
                        value={this.state.associate_bio.ielts}
                      />
                    )}
              ...
              <div className="details-side-panel">
                {this.state.associate_bio.profile_image_ref ? (
                  <div className="pro-img">
                    <img
                      ...
                      alt={
                        this.state.associate_bio.user_name + " profile image"
                      }
                    />
                  </div>
                ) : (
                ...
                <Query
                  query={ASSOCIATE_RATING}
                  variables={{
                    associate_id: this.state.data.id,
                  }}
                >
                  {({ loading, error, data }) => {
                    if (loading)
                      return <Spinner className="bp3-intent-success" />
                    if (error) return <div>Failed to load data</div>
                    return <RatingCard ratings={data.associateRating} />
                  }}
                </Query>
